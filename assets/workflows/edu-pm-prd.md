...
5. **极度重要（静态化支持）：** 页面内建悬浮导出按钮 `📸 一键导出所有截图`，仅在平铺模式下显示。导出引擎优先使用 `html-to-image` + `jszip`，不要默认使用 `html2canvas`。
6. **极度重要（保存路径）：** 导出时优先通过 `showDirectoryPicker` 将 PNG 直接保存到项目根目录的 `原型截图/` 目录；首次让用户选择一次该目录，后续通过 `indexedDB` 记住目录句柄并直接写入。仅当浏览器不支持目录写入时，才回退为 ZIP 下载。
7. **极度重要（图标兜底）：** 对 `Font Awesome` 图标必须提供导出兼容层。导出瞬间将关键 `fa-*` 图标替换为内联 SVG 或稳定文本符号，导出完成后立即恢复原 DOM，避免 PNG 中出现空心底框或图标丢失。
